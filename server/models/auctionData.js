...
class User {
    async createAproduct(product) {
        ...
        let results =
            `INSERT INTO products
             (name,price,date,picture,hour,winners,target,status,type,tickets,current, vendor, selling,published,win_t) VALUES
        ...
        let ticketsGroup = generateNums(product.tickets)

        let winnigTickets = checkWinner(JSON.parse(ticketsGroup), product.winners)


        let inserts = [product.name, product.price, product.date, product.picture, product.hour, product.winners, product.target, "current", product.type, product.tickets, ticketsGroup, product.vendor || "admin", product.selling, new Date, JSON.stringify(winnigTickets.map(n => JSON.parse(ticketsGroup)[n]))]
    ...
    async bid(createdBy, bid, user, country1) {
        ...
        JSON.parse(bid.bids).forEach(n => {
            if (JSON.parse(productsInfo.rows[0].win_t).some(x => n == x)) {
                luckyOnes.push(n)
            } else {
                lostOnes.push(n)
            }
        ...
        if (luckyOnes.length > 0) {
            let InsertWinner =
                `INSERT INTO winners
            (name,username,age,email,product,fortune,date,country, pro_id) VALUES
            ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING * 
           `
            const { firstname, secondname, age, email, country } = user
            let detailx = [firstname, secondname, age, email, prodName, luckyOnes.toString().split(',').join('-'), inputs3[5], country, bid.productid]
                ...
                let email2 = {
                    ...
                    <p>Congratulations ${secondname}! You’re the winner of ${prodName}. Your ticket number
                        ${luckyOnes.toString().split(',').join('-')} <strong>emerged as lucky winning ticket</strong> after going through our unbias
                            ...
                            <img src="https://i.imgur.com/JAXTYOG.png" width="20px" height="20px" alt="Image Loading">
            ...
            await client.query(InsertWinner, detailx)
        ...
        if (lostOnes.length > 0) {
            ...
            let InsertWinners2 =
               ...
               `
            const { firstname, secondname, email, age, country } = user
            let detailz = [firstname, secondname, age, email, prodName, lostOnes.toString().split(',').join('-'), inputs3[5], country]
            ...
            await client.query(InsertWinners2, detailz)
    ...
    async deleteAuction(id, status) {
        ...
        await client.query('delete from winners where pro_id=$1', [id])
